<template>
  <div>
    <div v-if="medications.length <= 0" class="text-sm text-gray-500 text-center">No medications on record</div>
    <div v-if="activeMedications.length > 0" class="mt-4">
      <h2 class="text-gray-500 text-xs font-medium uppercase tracking-wide">Active</h2>
    ...
    <div v-if="inactiveMedications.length > 0" class="mt-4">
      <h2 class="text-gray-500 text-xs font-medium uppercase tracking-wide">Inactive</h2>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    useFetch(async () => {
      const res: Medication[] = await $axios.$get(props.record.links.medications)
